...
function OderInfo({ params }: { params: any }) {
  ...
  const itemsColumns = [
    ...
    },
    {
      title: 'Price',
      dataIndex: 'price',
      render: (_: number, record: Product) => `${formatPrice(record.price)}`
    },
    {
      title: 'Quantity',
      dataIndex: 'quantity'
      ...
      {order && (
        <div>
          ...
          <div className='flex flex-col xl:grid grid-cols-3 gap-5 mt-8 w-full'>
            ...
            {getProperty('Total Amount', `${formatPrice(order.total)}`)}
            {getProperty('Order Status', capitalize(order.orderStatus))}
            {getProperty('Payment Status', capitalize(order.paymentStatus))}
